	...
	switch ( listenerName )
	{
		...
		case 'Table' :
			...
			AddItems : function( menu, tag, tagName )
			{
				...
				var bIsCell		= false;//( !bIsTable && FCKSelection.HasAncestorNode( 'TABLE' ) ) ;
		...
		case 'Button' :
			...
			AddItems : function( menu, tag, tagName )
			{
				return;
				if (!tag) return;
				if (tagName == 'INPUT' && ( tag.type == 'button' || tag.type == 'submit' || tag.type == 'reset' ) && !ctrltype)
				{
					menu.AddSeparator() ;
					menu.AddItem( 'Button', FCKLang.ButtonProp, 54 ) ;
				}

				var ctrltype = tag.getAttribute('ctrltype');
				if (ctrltype)	{
					switch (ctrltype) {
						case "FieldTag"		:	menu.AddItem( 'FieldTag', FCKLang.FieldTag, 5 ) ; break;
						case "ActionTag"	:	menu.AddItem( 'ActionTag', FCKLang.ActionTag, 5 ) ; break;
						case "TagTitle"		:	menu.AddItem( 'TagTitle', FCKLang.FieldTitleTag, 5 ) ; break;

						case "ColumnTitleTag"		:	menu.AddItem( 'ColumnTitleTag', FCKLang.ColumnTitleTag, 5 ) ; break;
						case "QueryTag"		:	menu.AddItem( 'QueryTag', FCKLang.QueryTag, 5 ) ; break;
						case "ColumnActionTag"		:	menu.AddItem( 'ColumnActionTag', FCKLang.ColumnActionTag, 5 ) ; break;
						case "ColumnTag"		:	menu.AddItem( 'ColumnTag', FCKLang.ColumnTag, 5 ) ; break;
						case "IncludeTag"		:	menu.AddItem( 'IncludeTag', FCKLang.IncludeTag, 5 ) ; break;
						case "ContentTag"		:	menu.AddItem( 'ContentTag', FCKLang.ContentTag, 5 ) ; break;
					}
				}
			}} ;
