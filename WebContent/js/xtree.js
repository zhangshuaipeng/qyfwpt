...
WebFXTree.prototype.toString = function() {
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\">" +
		...
		(this.target && this.action!=webFXTreeConfig.defaultAction ? " target=\"" + this.target + "\"" : "") +
		" onclick=\"" + (this.action==webFXTreeConfig.defaultAction ? "webFXTreeHandler.toggle(this.parentNode);" : "return runCmd(this,'" + this.nodeId + "','" + this.icon + "');") + "\">" + this.text + "</a></div>" +
...
function runCmd(obj,id,icon) {
	...
	if (obj.href.indexOf("javascript:")==0 || obj.href=="#") {
		return false;
	}
	// 弹出窗口
	if (target=="DIALOG")	{
		top.window.showDialog(window,obj.innerHTML,obj.href,"640","480");
		return false;
	}
	// 在页签中运行
	if (target=="TAB") {
		...
		self.top.tab.addTab(id,obj.innerHTML,obj.href,icon,true);
		return false;
	}
	// 在框架中运行
	if (target.indexOf("FRAME")==0) {
		...
		window.parent.dhxLayout.items[nIndex].attachURL(obj.href);
		return false;
	}
...
WebFXTreeItem.prototype.setNodeId = function(nodeId) {
	this.nodeId = nodeId;
}
...
//	alert(label);
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\">" +
		...
		"<a href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" +
		(this.target && this.action!=webFXTreeConfig.defaultAction ? " target=\"" + this.target + "\"" : "") +
		" onclick=\"" + (this.action==webFXTreeConfig.defaultAction ? "webFXTreeHandler.toggle(this.parentNode);" : "return runCmd(this,'" + this.nodeId + "','" + this.icon + "');") + "\">" + label + "</a></div>" +
		"<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
	var sb = [];
	for (var i = 0; i < this.childNodes.length; i++) {
		sb[i] = this.childNodes[i].toString(i,this.childNodes.length);
